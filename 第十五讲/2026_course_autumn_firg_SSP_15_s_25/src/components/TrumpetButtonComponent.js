...
export default class TrumpetButtonComponent {
     ...
     * @returns {{ image: any, sound: Phaser.Sound.BaseSound, play: Function, stop: Function, setVisible: Function, setSoundKey: Function, showAndPlay: Function, showIdle: Function }}
     */
    static create(scene, options = {}) {
        ...
        let currentSoundKey = options.soundKey || 'title1';
        const autoPlay = options.autoPlay === true;
        const visible = options.visible !== false;

        let sound = scene.sound.add(currentSoundKey);
        ...
        const bindSoundEvents = (targetSound) => {
            targetSound.on('complete', playIdle);
            targetSound.on('stop', playIdle);
        };

        const unbindSoundEvents = (targetSound) => {
            targetSound.off('complete');
            targetSound.off('stop');
        };

        bindSoundEvents(sound);
        playIdle();
        ...
        return {
            ...
            setSoundKey: (newSoundKey) => {
                if (newSoundKey === currentSoundKey) return;
                unbindSoundEvents(sound);
                if (sound.isPlaying) sound.stop();
                sound.destroy();
                currentSoundKey = newSoundKey;
                sound = scene.sound.add(newSoundKey);
                bindSoundEvents(sound);
            },
            showAndPlay: () => {
