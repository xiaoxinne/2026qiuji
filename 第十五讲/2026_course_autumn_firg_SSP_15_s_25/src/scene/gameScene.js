...
/**
 * 目标图：3×3×3 正方体（447×419）
 * area1：1×3×3 竖切片（227×419）
 * 三片横向错位 AREA1_SLICE_DX 叠合，与目标图一致
 */
const CUBE_CENTER = { x: 978, y: 356 };
const AREA1_SLICE_DX = 110;
/** 第 2/3 关三竖列岔开间距（相对中间列，同 t_12） */
const SPLIT_GAP = 160;
const SPLIT_DURATION = 420;

/** 三关填空：3 个 area1（col 0/1/2）叠成 3×3×3 */
const LEVELS = [
    {
        titleKey: 'title1',
        titleX: 943,
...
export default class gameScene extends Phaser.Scene {
    constructor() {
        ...
        ReportHelper.gameReportParams.question_id = '2026_course_autumn_firg_SSP_15_s_25';
    ...
    create() {
        ...
        this.trumpet.image?.setDepth(26);
        ...
        this.numButtons.buttons?.forEach((btn) => btn.icon?.setDepth(25));
        this.numButtons.symbolButtons?.forEach((btn) => btn.icon?.setDepth(25));
        ...
        this.submitButton.getMainIcon().setDepth(25);
        ...
        this.clearButton.getMainIcon().setDepth(25);
    ...
    setupAnswerSlots(levelIndex) {
        ...
        this.answerSlots = slots.map((pos, index) => {
            const bg = this.add.image(pos.x, pos.y, 'answer_bg').setDepth(31);
    ...
    refreshSlotVisual(slot) {
        ...
        digits.forEach((d, i) => {
            ...
            if (!slot.srImages[i]) {
                slot.srImages[i] = this.add.image(x, slot.y, `sr_${d}`).setDepth(32);
            } else {
    ...
    /** 合拢位：三片叠成正方体；层级同 t_12：左侧先画、右侧后画 */
    setupLevelAreas(levelIndex) {
        this.clearAreaImages();
        const level = LEVELS[levelIndex];
        const areas = [...(level.areas || [])].sort((a, b) => a.col - b.col);

        this.areaImages = areas.map((area, index) => {
            const homeX = CUBE_CENTER.x + (area.col - 1) * AREA1_SLICE_DX;
            const homeY = CUBE_CENTER.y;
            const delta = area.col - 1;
            const startX = level.splitColumns && !level.splitAnimate
                ? homeX + delta * SPLIT_GAP
                : homeX;
            const img = this.add.image(startX, homeY, area.key);
        ...
        if (level.splitColumns && level.splitAnimate) {
    ...
    goToNextLevel() {
        ...
        this.titleImage.setPosition(this.currentLevel.titleX, this.currentLevel.titleY);
    ...
    playLevelTitle() {
        ...
        this.trumpet.setSoundKey?.(this.currentLevel.soundKey);
