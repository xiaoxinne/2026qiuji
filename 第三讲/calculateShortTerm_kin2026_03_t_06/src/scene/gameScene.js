...
export default class gameScene extends Phaser.Scene {
    ...
    _shouldShowNumPad() {
        if (this.currentStep === 1) {
            if (!this._isPlayDone() || this.step1Completed) return false;
    ...
    _syncInputEnabled() {
        const inputReadyStep1 = this.currentStep === 1
            && this._isPlayDone()
            && !this.step1Completed;
    ...
    _checkStep1Equation() {
        if (this.currentStep !== 1 || this.step1Completed || !this._isPlayDone()) {
            return;
        }
