...
export default class DragDropComponent {
     ...
     * config.showGhostOnDrop - 放入放置区后，在 home 显示半透明占位，点击可召回（默认 false）
     * config.ghostAlpha - ghost 透明度（默认 0.4）
     */
    constructor(scene, config = {}) {
        ...
        this.showGhostOnDrop = showGhostOnDrop;
    ...
    _createDragItem(config, index) {
        ...
        if (this.showGhostOnDrop && texture) {
            const ghost = this.scene.add.image(x, y, texture);
            ghost.setScale(scale);
            ghost.setAlpha(this.ghostAlpha);
            ghost.setDepth(this.baseDepth - 1);
            ghost.setVisible(false);
            ghost.setInteractive({ useHandCursor: true });
            ghost.on('pointerup', () => {
                if (!item.currentZone) return;
                this.scene.sound.play('btnclick');
                this.returnItem(item.key, true);
            });
            item.ghost = ghost;
        }
    ...
    _showGhost(item) {
        if (item?.ghost) {
            item.ghost.setVisible(true);
        }
    }

    _hideGhost(item) {
        if (item?.ghost) {
            item.ghost.setVisible(false);
        }
    }

    _syncGhostVisibility(item) {
        if (!item?.ghost) return;
        if (item.currentZone) {
            this._showGhost(item);
        } else {
            this._hideGhost(item);
        }
    }
    ...
    destroy() {
        this.items.forEach(item => {
            ...
            item.sprite.off('drag');
            item.sprite.off('dragend');
            if (item.ghost && !item.ghost.destroyed) {
                item.ghost.destroy();
            }
