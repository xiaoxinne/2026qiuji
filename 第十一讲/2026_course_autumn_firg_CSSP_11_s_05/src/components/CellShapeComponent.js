...
export default class CellShapeComponent {
  ...
  static create(scene, options = {}) {
    ...
    const dragLayout = matchZones.length
      ? CellShapeComponent._buildLayoutFromMatchZones(
        cells, minCol, minRow, matchZones, dragCellWidth, dragCellHeight,
      )
      : CellShapeComponent._buildLayout(
        cells, minCol, minRow, dragCellWidth, dragCellHeight, -3, -3,
      );
    ...
    if (firemanIndex >= 0 && firemanIndex < cells.length) {
      ...
      firemanSprite = scene.add.image(pos.x, pos.y, firemanTexture);
    ...
    if (firemanIndex >= 0 && firemanIndex < cells.length) {
      const pos = normalLayout.positions[firemanIndex];
      ghostFireman = scene.add.image(pos.x, pos.y, firemanTexture);
    ...
    const applyLayout = (layout, mode) => {
      ...
      if (firemanSprite) {
        ...
        const useDrag = mode === 'dragging' || mode === 'matched';
        firemanSprite.setTexture(useDrag ? firemanDragTexture : firemanTexture);
    ...
    const setDragEnabled = (enabled) => {
      dragEnabled = enabled;
      if (enabled) {
        updateInteractive(matched ? dragLayout : normalLayout);
      } else {
    ...
    container.on('dragstart', () => {
      ...
      applyLayout(dragLayout, 'dragging');
    ...
    container.on('dragend', () => {
      ...
      const snap = CellShapeComponent._findMatch(
        container, dragLayout, cells, matchZones, matchThreshold,
      );
